...
Вставьте её в чат **как reply (ответ) на сообщение бота** (без команды).

> **Важно:** в группах бот получает обычные сообщения только если отключен Privacy Mode (BotFather → Bot Settings → Group Privacy → Turn off). Если Privacy Mode включён — отправляйте ссылку как **reply** на любое сообщение бота.
...
Вы:    [reply на сообщение бота] https://claude.ai/login?token=xyz...
