...
## Запуск через VPN (WireGuard)

Весь исходящий трафик бота — и headless-браузер, и Telegram — можно увести в
VPN-туннель WireGuard. Наружу тогда виден адрес VPN-сервера, а не реальный адрес
хоста. Режим выбирается тем, какой compose-файл запускаешь.

| Режим | Команда | Что делает |
|---|---|---|
| Без VPN (по умолчанию) | `docker compose up -d` | Трафик напрямую с адреса хоста. |
| Через VPN | `docker compose -f docker-compose.vpn.yml up -d` | Трафик через туннель. |

Оба файла делят один том `bot_data` — база, сессии и логи общие при переключении
режима. Одновременно оба режима не запустить (бот всегда один).

**Как включить VPN:**

1. Скопируй шаблон и подставь значения своего VPN-сервера:
   ```bash
   cp wireguard/wg0.conf.example wireguard/wg0.conf
   # отредактируй wireguard/wg0.conf (ключи, Endpoint; AllowedIPs = 0.0.0.0/0)
   ```
   Файл `wireguard/wg0.conf` — секрет, в git не попадает (см. `.gitignore`).

2. Запусти VPN-режим:
   ```bash
   docker compose -f docker-compose.vpn.yml up -d
   # или короче: task up:vpn
   ```
   Сначала поднимается контейнер `wireguard`, устанавливает туннель и по `wg show`
   получает статус healthy. Только после этого стартует `bot` (в сетевом
   пространстве `wireguard`).

3. Проверь, что трафик идёт через VPN — внешний адрес бота должен быть адресом
   VPN-сервера:
   ```bash
   task vpn:ip        # внешний адрес бота (через wget — в образе нет curl)
   task vpn:status    # состояние туннеля и рукопожатие
   ```

**Защита от утечки (kill-switch).** Единственный выход бота наружу — сеть
VPN-клиента. Нет туннеля (упал, не поднялся, нет файла `wg0.conf`) — бот теряет
интернет целиком и не стартует; реальный адрес хоста не утекает ни на старте, ни
при разрыве.
